...
import { setColumnsAndRows, addRow, deleteRow, updateRowStatus, openModal, closeModal } from './features/tableSlice';
...
const App: React.FC = () => {
  ...
  const { columns, rows, obj } = useSelector((state: RootState) => state.table);
  ...
  return (
    <div className='main'>
      ...
      <Modal
        ...
        onEditHandler={() =>
          dispatch(updateRowStatus({ obj.rowId, obj.selectedCol, obj.status }))
        }
      />
    </div>
  );
